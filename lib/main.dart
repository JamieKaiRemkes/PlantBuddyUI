...
class _MyAppState extends State<MyApp> {
  ...
  Future<void> startDiscovery() async {
    ...
    await flutterNsd.discoverServices('_plantbuddy32._tcp.');
  }
    ...
    if (services.isEmpty && _scanning) {
      ...
      return ListView.builder(
        itemBuilder: (context, index) => Column(
          children: [
            Text((services[index].name ?? 'Invalid service name')),
            Text((services[index].txt.toString() ?? 'Invalid service name')),
            Text((services[index].hostname ?? 'Invalid service name')),
            Text((services[index].port.toString() ?? 'Invalid service name')),
            Text((services[index].toString() ?? 'Invalid service name')),
          ],
        ),
